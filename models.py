...
def extract_peak(heatmap, max_pool_ks=7, min_score=-5, max_det=100):
    ...
    top_scores, top_indices = torch.topk(torch.where(is_peak > 0, heatmap, is_peak).view(-1), k = height * width)
    for length, peak in enumerate(top_scores):
        if peak > min_score and length < max_det:
            list_peaks.append((float(peak), int(top_indices[length] % width), int(top_indices[length] // width)))
        else: 
            return list_peaks
    return list_peaks
...
class Detector(torch.nn.Module):
    ...
    def detect(self, image):
        ...
        """
        list_detections = []
        image = torch.sigmoid(self.forward(image[None]))

        for i in range(3): 
            detections = []
            peaks = extract_peak(image[0, i], min_score=0.1, max_det = 30)
            for peak in peaks:
                detections.append((peak[0], peak[1], peak[2], float(0), float(0)))
